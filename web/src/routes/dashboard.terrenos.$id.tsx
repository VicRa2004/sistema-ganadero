...
import { useObtenerDetalleTerreno } from "@/modules/terreno/hooks/useObtenerDetalleTerreno";
import { useObtenerCapacidadTerreno } from "@/modules/terreno/hooks/useObtenerCapacidadTerreno";
...
export const Route = createFileRoute("/dashboard/terrenos/$id")({
	beforeLoad: ({ context }) => {
	...
	component: DetalleTerrenoComponent,
});

function DetalleTerrenoComponent() {
	const { id } = Route.useParams();
	const terrenoId = Number(id);

	const permissions = useAuthStore((state) => state.permissions) ?? [];
	const canUpdate = permissions.includes("terreno:update");

	const {
		data: terreno,
		isLoading: isLoadingDetalle,
		isError: isErrorDetalle,
	} = useObtenerDetalleTerreno(terrenoId);

	const {
		data: capacidad,
		isLoading: isLoadingCapacidad,
		isError: isErrorCapacidad,
	} = useObtenerCapacidadTerreno(terrenoId);
	...
	if (isLoading) {
		return (
			<div className="flex items-center justify-center py-24 gap-3 text-muted-foreground">
				...
				<span className="text-sm">Cargando información del terreno...</span>
			</div>
		);
	}

	if (isError || !terreno || !capacidad) {
		return (
			<div className="space-y-4 py-6 animate-fade-in">
				<Link
					to="/dashboard/terrenos"
					className={cn(
				...
				<div className="p-4 text-sm text-red-700 bg-red-500/10 border border-red-500/30 rounded-xl dark:text-red-400 dark:bg-red-500/15">
					No se pudo cargar la información del terreno. Verifica que exista o
					regresa a la lista.
				</div>
			</div>
		);
	...
	return (
		<div className="space-y-8 py-6 animate-fade-in">
			...
			<Card className="border border-border bg-card/60 shadow-sm relative overflow-hidden">
				...
				<CardHeader className="flex flex-col sm:flex-row items-start gap-4 pb-4">
					{terreno.imagenTerreno ? (
						<img
							src={`${baseApiUrl}${terreno.imagenTerreno}`}
							alt={terreno.nombre}
							className="size-16 sm:size-20 rounded-xl object-cover border bg-background shrink-0 shadow-sm"
						/>
					) : (
					...
					<div className="text-left space-y-1">
						...
						<div className="flex flex-wrap gap-2 pt-1">
							<Badge variant="secondary" className="gap-1.5">
								...
								{terreno.extensionHectareas} Hectáreas
							</Badge>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border/50">
							...
							<div className="text-left">
								...
								<p className="text-sm font-medium">{terreno.ubicacion}</p>
							</div>
						</div>
						<div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border/50">
							...
							<div className="text-left">
								...
								<p className="text-sm font-medium">
									{terreno.capacidadMaxima} cabezas
								</p>
							</div>
						</div>
					</div>
			...
			<Card className="border border-border bg-card/40 shadow-sm">
				<CardHeader>
					<CardTitle className="text-lg font-bold text-foreground text-left">
						Estado de Capacidad
					</CardTitle>
				</CardHeader>
				<CardContent className="space-y-6">
					...
					<div className="space-y-2">
						...
						{ocupacionPorcentaje >= 90 ? (
							<p className="text-xs text-rose-500 font-medium text-left">
								⚠️ El terreno está al límite de su capacidad máxima permitida.
								Evita ingresar más ganado.
							</p>
						) : ocupacionPorcentaje >= 70 ? (
							<p className="text-xs text-amber-500 font-medium text-left">
								💡 La capacidad se está acercando al límite recomendado.
							</p>
						) : (
							<p className="text-xs text-emerald-500 font-medium text-left">
